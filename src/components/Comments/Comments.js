...
export default function SimpleAccordion({setCurrentId ,post}) {
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ title: '', message: '', tags: '', selectedFile: '',profilepic:'',Comments:'' });
  };
    ...
    <div className={classes.root}>
      <Accordion  elevation={0}>
        ...
        <div className={classes.comments1}></div>
          ...
          <div className={classes.root}>
          <form autoComplete="off" noValidate  onSubmit={() =>{ dispatch(CommentPost(post._id, { ...postData}));clear();}}>
          <TextField className={classes.input} name="comment" variant="outlined" label="comment" fullWidth  onChange={(e) => setPostData({ ...postData, Comments:{comment:e.target.value,commenterpic:user.result.profilepic ||user?.result.imageUrl}  })} />
